          ...
          <div v-if="mapShow">
            ...
            <a-form-item label="统计项" :style="{ marginTop: '29px' }">
              <a-select
                ...
                :style="{ width: '360px', height: '32px', marginLeft: '15px' }"
              >
                <a-select-option v-for="item in statisData" :key="item.id">{{ item.item_name }}</a-select-option>
              </a-select>
            </a-form-item>
          </div>
...
export default {
  ...
  created() {
    ...
    // 获取基础图表下拉框列表
    setTimeout(() => {this.getChartOptionData()}, 100)
    //获取维度下拉框列表
    setTimeout(() => {this.getDimensionData()}, 200)
    //获取统计项(写在获取数据源ID接口中)
    // setTimeout(() => {this.getDataSourceId()}, 0)
    setTimeout(() => {this.getDataSourceId()}, 300)
    // this.getDataSetList()
    //编辑回显
